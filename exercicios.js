...
function exercicio1() {
    ...
    const result = document.getElementById("resultado-ex1");
        result.textContent = "Titulo encontrado: " + titulo.textContent;
...
function exercicio2() {
  ...
  const result = document.getElementById("resultado-ex2")
  ...
  if (favoritado) {
    ...
    result.textContent = "💖 Breaking Bad foi favoritado";
  } 
  //       else            { remover classe,  mudar botão e resultado }
  else {
   card.classList.remove("card-favoritado-ex");
   btn.textContent = "💔 Desfavoritado";
   result.textContent = "💔 Breakin Bad foi desfavoritado";
  }
}
